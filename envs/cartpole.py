...
from utils.utils import conditional_fork_rng, bsolve


class CartPole():
    def __init__(self, parameters, noise_std, x_min, x_max, u_min, u_max, bs, barrier_thresh, max_steps, device="cuda:0", random_seed=None, quiet=False, keep_stats=False, run_name=None):
        """
        ...
        self.m_car_min = parameters["m_car"][0]
        self.m_car_max = parameters["m_car"][1]
        ...
        self.m_car = self.m_car_min * batch_ones(tuple())
        self.l = self.l_min * batch_ones(tuple())
        self.m_pole_nom = 0.5 * (self.m_pole_min + self.m_pole_max) * single_ones(tuple())
        self.m_car_nom = 0.5 * (self.m_car_min + self.m_car_max) * single_ones(tuple())
    ...
    def obs(self):
        """Returns the observation from the environment in (x, x_dot, sin(theta), cos(theta), theta_dot)."""
        return torch.cat([t.unsqueeze(-1) for t in [self.x, self.x_dot, torch.sin(self.theta), torch.cos(self.theta), self.theta_dot]], dim=-1)

    def cost(self):
        """Computes and returns the cost based on the state and control input."""
        return (self.x - self.x_ref) ** 2 + self.x_dot ** 2 + self.theta ** 2 + self.theta_dot ** 2 + 0.1 * self.u ** 2
    ...
    def reset_done_envs(self, need_reset=None, x=None, x_ref=None, randomize_seed=None):
        ...
        with conditional_fork_rng(seed=randomize_seed, condition=randomize_seed is not None):
            ...
            self.m_car[is_done] = self.m_car_min + (self.m_car_max - self.m_car_min) * torch.rand((size,), device=self.device)
    ...
    def reset(self, x=None, x_ref=None, randomize_seed=None):
        ...
        self.reset_done_envs(x=x, x_ref=x_ref, randomize_seed=randomize_seed)
        return self.obs()
    ...
    def step(self, u):
        ...
        lhs_mat = torch.stack([
            torch.stack([self.m_car + self.m_pole, self.m_pole * self.l * torch.cos(self.theta)], dim=-1),
        ...
        rhs_vec = torch.stack([
            self.u + self.m_pole * self.l * self.theta_dot ** 2 * torch.sin(self.theta),
        ...
        # Write episode stats
        if self.keep_stats:
            done_indices = torch.nonzero((self.is_done == 1 | self.is_done == 2) & (self.already_on_stats == 0), as_tuple=False)
